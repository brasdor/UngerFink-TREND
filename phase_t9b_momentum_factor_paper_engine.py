...
def compute_basket(close: pd.DataFrame, as_of: Date) -> tuple[list[str], list[str], dict]:
    """
    ...
    if len(sig_short) < SHORT_K:
        return [], [], meta   # cannot fill short basket
    ...
    meta['actual_long_k'] = actual_long_k
    meta['long_min_ret']  = (round(float(sig_long.nlargest(actual_long_k).iloc[-1]) * 100, 2)
                             if actual_long_k > 0 else None)
    meta['short_max_ret'] = round(float(sig_short.nsmallest(SHORT_K).iloc[-1]) * 100, 2)
    if actual_long_k < LONG_K:
        meta['long_note'] = (f"Only {actual_long_k}/{LONG_K} symbols above EMA200 "
                             f"-- bear market condition, {actual_long_k} longs opened")
...
def do_rebalance(state: dict, close: pd.DataFrame, today: Date) -> list[dict]:
    """
    ...
    # New baskets
    longs, shorts, meta = compute_basket(close, today)
...
def main() -> None:
    ...
    if is_rebal_day(today, state):
        p(f"\n  REBALANCE DAY")
        signal_rows = do_rebalance(state, close, today)
        rebalanced  = True
    else:
        days_since = 0
        if state['last_rebal_date']:
        ...
        next_friday = today
        while next_friday.weekday() != 4 or (today - next_friday).days < 0:
