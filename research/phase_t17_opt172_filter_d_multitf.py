...
# -*- coding: utf-8 -*-
"""
Phase T17 Opt-17.2 -- Regime Filter D: Multi-Timeframe Weekly EMA200

Filter applied at entry: weekly close must be above the weekly EMA(200).

Implementation (no lookahead):
  1. Resample daily OHLCV to weekly bars (week ending Sunday, pandas 'W').
  2. Compute EMA(200) on weekly close prices.
  3. For each weekly bar: weekly_above = (weekly_close > weekly_ema200).
  4. Forward-fill weekly_above to daily resolution.
     Each daily bar inherits the status of the most recently completed weekly bar.
  5. At entry on daily bar i: require weekly_above[i] == True.

Warmup note:
  Weekly EMA(200) needs 200 weekly bars = ~3.8 years.
  With data starting 2020-05-20, the filter activates from ~2024-03-17.
  Trades before that date are blocked by the warmup (treated as filter=False).
  This is an irreducible cost of using a 200-week lookback.
...
Output: data/research_donchian_regimeV2_multitf/
"""
...
OUT_DIR    = ROOT / "data" / "research_donchian_regimeV2_multitf"
...
def compute_weekly_ema200_gate(df: pd.DataFrame) -> Tuple[np.ndarray, dict]:
    """
    Build a daily boolean array: True when the most recently completed weekly
    bar's close is above the weekly EMA(200).  No lookahead bias.

    Strategy:
      - Resample daily bars to weekly (pandas 'W' = week ending Sunday).
      - Compute EMA(200) on the resulting weekly close series.
      - For each daily bar, forward-fill from the most recent completed
        weekly bar (i.e., most recent Sunday whose timestamp is <= bar date).
      - NaN entries (EMA warmup period) are treated as False (no entry).

    Returns:
      gate      : np.ndarray[bool], shape (len(df),)
      stats     : dict with diagnostic counts
    """
    df_idx = df.set_index("time").sort_index()

    # Weekly close: last bar of each calendar week (week ending Sunday)
    weekly_close = df_idx["close"].resample("W").last().dropna()

    if len(weekly_close) < WEEKLY_EMA_N + 5:
        # Insufficient history — gate is always False
        return (np.zeros(len(df), dtype=bool),
                dict(weekly_bars=len(weekly_close), warmup_bars=len(df),
                     valid_bars=0, above_bars=0, above_pct=0.0,
                     ema_valid_from="n/a"))

    # EMA(200) on weekly close
    wc      = weekly_close.to_numpy(dtype=float)
    w_ema   = _ema(wc, WEEKLY_EMA_N)
    w_above = pd.Series(
        wc > w_ema,                # True/False; NaN positions are False
        index=weekly_close.index,
        dtype=object,
    )
    # Replace positions where EMA is NaN with NaN so ffill skips them
    w_above[~np.isfinite(w_ema)] = np.nan

    # Forward-fill weekly signal to daily resolution.
    # For daily bar on date D: inherit signal from most recent Sunday S <= D.
    # This is correct because:
    #   - On Sunday D: weekly bar including D is completed; we use D's close (known).
    #   - Mon–Sat: most recent Sunday is the previous one (already completed). ✓
    daily_gate = (w_above
    ...
    # Diagnostics
    first_valid_week = weekly_close.index[WEEKLY_EMA_N - 1]
    warmup_mask  = df_idx.index < first_valid_week
    valid_mask   = ~warmup_mask
    above_mask   = daily_gate & valid_mask
    ...
    stats = dict(
        ...
        ema_valid_from= str(first_valid_week.date()),
    )

    return daily_gate.to_numpy(dtype=bool), stats
...
def run_backtest(
    ...
    """Donchian Long backtest with optional weekly EMA200 confirmation gate."""
    df = df.sort_values("time").reset_index(drop=True)
    nb = len(df)
    min_bars = max(entry_n, ATR_N, 200) + 20
    if nb < min_bars:
        return []
    ...
    # Pre-compute weekly gate for this symbol
    if apply_weekly:
        weekly_gate, _ = compute_weekly_ema200_gate(df)
    else:
        weekly_gate = np.ones(nb, dtype=bool)
    ...
    for i in range(1, nb):
        ...
        if pos is None:
            ...
            if not breakout:
                continue
            if apply_weekly and not weekly_gate[i]:
                continue
...
def run_universe(entry_n: int, apply_filter: bool, symbols: List[str]) -> pd.DataFrame:
    ...
    for sym in symbols:
        ...
        if df is None or len(df) < 250:
            continue
        trades = run_backtest(df, sym, entry_n=entry_n, apply_weekly=apply_filter)
...
# =============================================================================
# WEEKLY GATE DIAGNOSTICS (aggregate across all symbols)
# =============================================================================

def compute_weekly_gate_summary(symbols: List[str]) -> dict:
    """Aggregate weekly EMA200 gate stats across all 24 symbols."""
    warmup_total = valid_total = above_total = 0
    first_valid_dates = []
    ...
    for sym in symbols:
        ...
        if df is None or len(df) < 250:
            continue
        _, stats = compute_weekly_ema200_gate(df)
        warmup_total += stats["warmup_bars"]
        valid_total  += stats["valid_bars"]
        above_total  += stats["above_bars"]
        if stats["ema_valid_from"] != "n/a":
            first_valid_dates.append(stats["ema_valid_from"])

    earliest_valid = min(first_valid_dates) if first_valid_dates else "n/a"
    latest_valid   = max(first_valid_dates) if first_valid_dates else "n/a"
    return dict(
        warmup_total    = warmup_total,
        valid_total     = valid_total,
        above_total     = above_total,
        above_pct       = above_total / valid_total if valid_total else 0.0,
        ema_valid_from_earliest = earliest_valid,
        ema_valid_from_latest   = latest_valid,
    )
...
def write_master_report(
    ...
    lines = [
        ...
        "=" * 70,
        "",
        f"Filter   : weekly close > weekly EMA({WEEKLY_EMA_N})",
        "Universe : 24 symbols (filtered_symbols_v2_included_only.csv)",
        ...
        "",
        "=" * 70,
        f"WEEKLY EMA{WEEKLY_EMA_N} GATE DIAGNOSTICS",
        "=" * 70,
        f"  Weekly EMA valid from (earliest) : {weekly_stats['ema_valid_from_earliest']}",
        f"  Weekly EMA valid from (latest)   : {weekly_stats['ema_valid_from_latest']}",
        f"  Warmup bars blocked (all symbols): {weekly_stats['warmup_total']:,}",
        f"  Valid bars (after warmup)        : {weekly_stats['valid_total']:,}",
        f"  Valid bars with signal=True      : {weekly_stats['above_total']:,}  "
        f"({weekly_stats['above_pct']:.1%})",
        f"  Note: 200-week warmup (~3.8yr) blocks all pre-2024-03 entries.",
        f"        Only ~2 years of signal data are active.",
        "",
        "=" * 70,
        "STABILITY ZONE  (N=[15, 20, 25], weekly EMA200 filter ON)",
        "=" * 70,
    ...
    lines += [
        ...
        "",
        "=" * 70, "T4 -- BASELINE (N=20, weekly EMA200 filter ON)", "=" * 70,
    ...
    lines += [
        "",
        "=" * 70, "FINAL SCORECARD -- Filter D vs Baseline", "=" * 70, "",
    ...
    lines += [
        ...
        "",
        "  STRUCTURAL NOTE: 200-week warmup blocks ~3.8 years of data.",
        "  Results reflect ~2 years of active signal (2024-03 to 2026-05).",
        "  CAGR comparison should be read with this context in mind.",
        "",
        f"  OVERALL: {'PASS -- Filter D improves or maintains quality' if all_gates else 'FAIL -- one or more gates failed'}",
    ]
...
def main() -> int:
    print("=" * 70)
    print("Phase T17 Opt-17.2 -- Regime Filter D: Multi-Timeframe Weekly EMA200")
    print("=" * 70)

    symbols = pd.read_csv(IN_SYMBOLS)["symbol"].tolist()
    print(f"Universe : {len(symbols)} symbols")
    print(f"Filter   : weekly close > weekly EMA({WEEKLY_EMA_N})")
    print(f"Warmup   : {WEEKLY_EMA_N} weeks (~{WEEKLY_EMA_N * 7 / 365.25:.1f} yr)")
    print()

    # ── WEEKLY GATE DIAGNOSTICS ──────────────────────────────────────────────
    print("[0] Computing weekly EMA200 gate diagnostics ...")
    weekly_stats = compute_weekly_gate_summary(symbols)
    print(f"  EMA valid from (first symbol): {weekly_stats['ema_valid_from_earliest']}")
    print(f"  Valid bars above EMA          : {weekly_stats['above_total']:,} / "
          f"{weekly_stats['valid_total']:,}  ({weekly_stats['above_pct']:.1%})")

    # ── STABILITY ZONE ───────────────────────────────────────────────────────
    print("\n[1] Stability zone (N=15/20/25, weekly EMA200 filter ON) ...")
    stab_rows = []
    ...
    # ── CANONICAL N=20 ───────────────────────────────────────────────────────
    print(f"\n[2] Canonical N={CANONICAL_N} with weekly EMA200 filter ...")
    ...
    mc.to_csv(OUT_DIR / "filter_d_t4_montecarlo.csv", index=False)
    cost.to_csv(OUT_DIR / "filter_d_t4_cost_stress.csv", index=False)
    splits.to_csv(OUT_DIR / "filter_d_t4_period_splits.csv", index=False)
    rem_assets.to_csv(OUT_DIR / "filter_d_t4_remove_best_assets.csv", index=False)
    rem_months.to_csv(OUT_DIR / "filter_d_t4_remove_best_months.csv", index=False)
    ...
    if not acc.empty:
        ...
        eq_df.to_csv(OUT_DIR / f"filter_d_t6_equity_max{PORT_MAX_OPEN}.csv", index=False)
