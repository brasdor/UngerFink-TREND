...
# -*- coding: utf-8 -*-
"""
Phase T4 -- DualMA Robustness Engine

Reads Phase T3B DualMA trades (best variant: chan_mult=3.0, act=4.0R) and
runs the standard Unger T4 stress battery:
  - Baseline summary
  - Block bootstrap Monte Carlo
  - Extra cost stress
  - Rolling calendar windows
  - Period splits (first/second half, last 100/200)
  - Remove best assets
  - Remove best months
  - Asset concentration

Input:  data/research_dualma_t3b/phase_t3b_wide_exit_trades.csv
Output: data/research_dualma_t4/
"""
...
INPUT_CANDIDATES = [
    PROJECT_ROOT / "data" / "research_dualma_t3b" / "phase_t3b_wide_exit_trades.csv",
    PROJECT_ROOT / "phase_t3b_wide_exit_trades.csv",
]

OUT_DIR = PROJECT_ROOT / "data" / "research_dualma_t4"
...
def normalize_trades(df: pd.DataFrame) -> pd.DataFrame:
    # Column mapping: DualMA T3B uses exit_bar instead of exit_time,
    # and does not include timeframe/side columns (fixed at 1d / LONG).
    if "exit_bar" in df.columns and TIME_COL not in df.columns:
        df = df.rename(columns={"exit_bar": TIME_COL})
...
def summarize_r(r_values: Iterable[float]) -> Dict[str, float]:
    ...
    if arr.size == 0:
        return {
            "trades": 0, "total_r": 0.0, "avg_r": 0.0, "median_r": 0.0,
            "win_rate": 0.0, "profit_factor": 0.0, "max_dd_r": 0.0,
            "best_trade_r": 0.0, "worst_trade_r": 0.0, "losing_streak": 0,
            "std_r": 0.0, "t_score": 0.0,
        }
    avg = float(arr.mean())
    ...
    for tf, side, g in group_slices(df):
        ...
        for block_size in MC_BLOCK_SIZES:
            ...
            for i in range(MC_RUNS):
                sample = block_bootstrap(values, block_size, rng)
                totals[i] = sample.sum()
                dds[i] = max_drawdown_r(sample)
                pfs[i] = profit_factor(sample)
            rows.append({
                "timeframe": tf, "side": side, "block_size": block_size,
...
def cost_stress_summary(df: pd.DataFrame) -> pd.DataFrame:
    ...
    for tf, side, g in group_slices(df):
        ...
        for extra_cost in EXTRA_COST_R_VALUES:
            ...
            s.update({"timeframe": tf, "side": side, "extra_cost_r_per_trade": extra_cost})
            rows.append(s)
...
def period_splits(df: pd.DataFrame) -> pd.DataFrame:
    ...
    for tf, side, g in group_slices(df):
        ...
        slices = {
            ...
            "first_half_by_time": g[g[TIME_COL] <= median_time],
            "second_half_by_time": g[g[TIME_COL] > median_time],
        }
...
def write_master_report(
    ...
    lines = []
    lines.append("PHASE T4 -- DUALMA ROBUSTNESS MASTER REPORT")
    lines.append("=" * 60)
    lines.append(f"Method:  DualMA (EMA30/EMA100, ema200_price filter)")
    lines.append(f"Config:  chandelier_atr_mult=3.0, chandelier_activate_r=4.0R")
    lines.append(f"Input:   {input_path}")
    lines.append(f"Trades:  {len(df)}")
    lines.append(f"Assets:  {df['symbol'].nunique()}")
    lines.append(f"Range:   {df[TIME_COL].min()} -> {df[TIME_COL].max()}")
    lines.append("")
    ...
    lines.append("")
    lines.append("CONCENTRATION NOTE")
    lines.append("-" * 60)
    lines.append("!! T3B flagged: BTC/USDT = 123.4% of total R (other assets collectively lose).")
    lines.append("The remove-best-assets table below is the key diagnostic.")
    lines.append("If removing BTC destroys the edge, the strategy is a BTC trade, not a portfolio.")
    lines.append("")

    lines.append("PERIOD SPLITS (1d ALL)")
    lines.append("-" * 60)
    sp1 = splits[(splits["timeframe"] == "1d") & (splits["side"] == "ALL")]
    for _, r in sp1.iterrows():
        lines.append(
            f"  {r['split']}: trades={int(r['trades'])}, "
    ...
    lines.append("- First/second half are not completely contradictory.")
    lines.append("- Rolling windows show multiple profitable periods.")
    lines.append("")
    lines.append("Bad signs (this system's known risk):")
    lines.append("- BTC concentration: removing BTC may destroy edge.")
    lines.append("- If so, consider BTC-only variant vs diversified variant in T5/T6.")
    lines.append("- MC p05 deeply negative = luck-dependent, not edge.")
    lines.append("- Recent last 100/200 trades structurally worse = decay.")
...
def main() -> int:
    ...
    print("=" * 70)
    print("PHASE T4 -- DUALMA ROBUSTNESS ENGINE")
    print("=" * 70)
    ...
    print("[OK] Monte Carlo (2000 runs x 5 block sizes)")
    ...
    print()
    print("=" * 70)
    print("T4 COMPLETE -- review phase_t4_master_report.txt before T5")
    print("=" * 70)
    return 0
