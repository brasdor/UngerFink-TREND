...
# -*- coding: utf-8 -*-
"""
Phase T1 -- ConsecDownDaysMR Concept Discovery
UngerFink Pipeline / Andrea Unger Methodology

Entry : price has closed DOWN for exactly consec_n consecutive days
        (streak reaches consec_n on that bar — enter on NEXT bar open)
Exit  : fixed time exit after hold_bars
Safety stop: ATR x atr_mult below entry close

Usage:
    python phase_t1_consecdowndays_mr_concept_discovery.py --timeframe 1d
    python phase_t1_consecdowndays_mr_concept_discovery.py  # all TFs
"""
...
OUT_DIR = ROOT / "data" / "research_consecdowndays_mr_t1"
...
def add_indicators(df: pd.DataFrame) -> pd.DataFrame:
    """Add ATR14, EMA200, and pre-compute consecutive down-day streak vector."""
    out   = df.copy()
    close = out["close"]
    high  = out["high"]
    low   = out["low"]
    ...
    # streak[i] = number of consecutive bars (ending at bar i) where close < prev_close
...
def backtest_symbol_all_combos(
    ...
    """Run all (consec_n, hold_bars, atr_mult) combos for one symbol."""
    ...
    for (consec_n, hold_bars, atr_mult) in param_combos:
        trades: list[dict] = []
        in_pos  = False
        e_price = 0.0
        stop    = 0.0
        ...
        for i in range(len(df)):
            if np.isnan(atr[i]):
                continue
            if not in_pos:
                # Enter when streak EXACTLY reaches consec_n
                # (streak[i]==consec_n means today is down, prior consec_n-1 were also down,
                #  and the bar before that was NOT down — first time hitting this streak length)
                if streak[i] == consec_n and filt[i]:
                    in_pos  = True
                    e_price = close[i]
                    e_bar   = i
        ...
        results[(consec_n, hold_bars, atr_mult)] = trades
    return results
...
def stability_score(grid: pd.DataFrame, tf: str, filt: str,
                    consec_n: int, hold_bars: int, atr_mult: float) -> float:
    ns = sorted(CONSEC_N)
    try:
        ci = ns.index(consec_n)
    except ValueError:
        return 0.0
    n_nbrs = [ns[j] for j in range(max(0,ci-1), min(len(ns),ci+2))]
    zone = grid[
        (grid["timeframe"]==tf) & (grid["filter_mode"]==filt) &
        (grid["consec_n"].isin(n_nbrs)) &
...
def run_grid_for_tf(tf: str) -> pd.DataFrame:
    param_combos = list(itertools.product(CONSEC_N, HOLD_BARS, ATR_MULTS))
    p(f"  Param combos per filter: {len(param_combos)} x {len(FILTERS)} = {len(param_combos)*len(FILTERS)}")
    ...
    for filt in FILTERS:
        for (consec_n, hold_bars, atr_mult) in param_combos:
            trades = combo_trades[(filt, consec_n, hold_bars, atr_mult)]
            m = calc_metrics(trades)
            rows.append({
                "timeframe": tf, "filter_mode": filt,
                "consec_n": consec_n, "hold_bars": hold_bars, "atr_mult": atr_mult,
...
def report(grid: pd.DataFrame, tf_label: str) -> bool:
    ...
    if viable.empty:
        top = grid.nlargest(10,"num_trades")[
            ["timeframe","filter_mode","consec_n","hold_bars","atr_mult","num_trades","avg_r"]]
        p("  FAIL: no combos meet min_trades. Top by trade count:")
        p(top.to_string(index=False)); return False
    ...
    if viable.empty:
        cands = grid[grid["num_trades"]>=MR_GATES["min_trades"]].nlargest(10,"avg_r")[
            ["timeframe","filter_mode","consec_n","hold_bars","atr_mult","num_trades","win_rate","avg_r"]]
        p("  FAIL: no combos pass gates. Best by avg_r:")
    ...
    p(top[["timeframe","filter_mode","consec_n","hold_bars","atr_mult",
           "num_trades","win_rate","avg_r","stability"]].to_string(index=False))

    # Heatmap: avg_r by (consec_n, hold_bars)
    ...
    pivot = grid[(grid["timeframe"]==best_tf)&(grid["filter_mode"]==best_filt)]\
        .groupby(["consec_n","hold_bars"])["avg_r"].mean().unstack("hold_bars")
    p(f"\n  avg_r heatmap (consec_n vs hold_bars) | TF={best_tf} filter={best_filt}:")
    ...
    with open(OUT_DIR / "phase_t1_summary.txt", "w", encoding="utf-8") as f:
        f.write(f"ConsecDownDaysMR T1 -- {tf_label}\n")
        ...
        f.write(stable.head(20)[["timeframe","filter_mode","consec_n","hold_bars",
...
def main() -> None:
    ...
    p("=" * 65)
    p("  Phase T1 -- ConsecDownDaysMR Concept Discovery")
    p(f"  Timeframes: {tfs}  |  Symbols: {len(SYMBOLS)}")
    p(f"  CONSEC_N: {CONSEC_N}  |  HOLD_BARS: {HOLD_BARS}")
    p(f"  ATR_MULTS: {ATR_MULTS}  |  FILTERS: {FILTERS}")
    p("=" * 65)
