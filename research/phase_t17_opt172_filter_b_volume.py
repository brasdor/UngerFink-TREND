...
# -*- coding: utf-8 -*-
"""
Phase T17 Opt-17.2 -- Regime Filter B: Volume Confirmation

Filter applied at entry: breakout bar volume > 1.5x 20-bar prior MA of volume.
No changes to stops, exits, or any other parameter.
...
Tests
  ...
  Portfolio/CAGR : T5 max8 + T6 capital execution for CAGR comparison

Baseline (DonchianLong_UniverseV2, N=20, no regime filter):
  avg_r=+1.101R  PF=3.072  CAGR=+14.3% (max8, T6)

Output: data/research_donchian_regimeV2_filter_b/
"""
...
OUT_DIR    = ROOT / "data" / "research_donchian_regimeV2_filter_b"
...
# Filter B
VOL_MA_N  = 20    # prior-bar MA window for volume
VOL_MULT  = 1.5   # entry bar volume must exceed this multiple of the MA
...
RISK_PCT       = 0.0025
KILL_SWITCH_DD = -0.35
PORT_MAX_OPEN  = 8   # max8 for CAGR comparison
...
MIN_TRADES = 80   # go/no-go gate on trade count
...
class Trade:
    ...
    mae_r:        float
    mfe_r:        float
    vol_filtered: bool    # True = passed volume filter
...
def run_backtest(
    ...
    """Donchian Long backtest with optional volume confirmation filter."""
    df = df.sort_values("time").reset_index(drop=True)
    nb = len(df)
    min_bars = max(entry_n, ATR_N, 200) + VOL_MA_N + 20
    if nb < min_bars:
        return []
    ...
    # Donchian bands: use prior bars only (shift(1) then rolling)
    don_upper = pd.Series(hi).shift(1).rolling(entry_n).max().to_numpy()
    don_lower = pd.Series(lo).shift(1).rolling(exit_n).min().to_numpy()

    # Volume MA: mean of prior VOL_MA_N bars (shift(1) so current bar excluded)
    vol_ma = pd.Series(vol).shift(1).rolling(VOL_MA_N).mean().to_numpy()
    ...
    for i in range(1, nb):
        ...
        if pos is None:
            ...
            if apply_vol:
                if not (np.isfinite(vol_ma[i]) and np.isfinite(vol[i])):
                    continue
                if vol[i] <= VOL_MULT * vol_ma[i]:
                    continue
...
def run_universe(entry_n: int, apply_vol: bool, symbols: List[str]) -> pd.DataFrame:
    """Run backtest for all symbols and return a single trades DataFrame."""
    all_trades: List[Trade] = []
    for sym in symbols:
        df = load_ohlcv(sym)
        if df is None or len(df) < 250:
            continue
        trades = run_backtest(df, sym, entry_n=entry_n, apply_vol=apply_vol)
...
# =============================================================================
# T5 PORTFOLIO REPLAY (max8, for CAGR)
# =============================================================================
...
def write_master_report(
    ...
    lines = [
        ...
        "=" * 70,
        "",
        "Filter   : entry bar volume > 1.5x prior-20-bar MA of volume",
        "Universe : 24 symbols (filtered_symbols_v2_included_only.csv)",
        ...
        "",
        "=" * 70, "STABILITY ZONE  (N=[15, 20, 25], volume filter ON)", "=" * 70,
    ...
    lines += [
        ...
        "",
        "=" * 70, "T4 -- BASELINE (N=20, volume filter ON)", "=" * 70,
    ...
    rem_m_ok  = float(rem_top1_month["total_r"]) > 0
    ...
    lines += [
        "",
        "=" * 70, "FINAL SCORECARD -- Filter B vs Baseline", "=" * 70, "",
        ...
        gate(rem_m_ok,         f"Remove top-1 month profitable (totalR={float(rem_top1_month['total_r']):+.1f}R)"),
        gate(kill_ok,          "Kill switch not fired"),
        "",
        "  vs BASELINE (no volume filter):",
        ...
        f"{'BETTER' if better_cagr else 'WORSE'}" if t6_stats else "  CAGR   : n/a (T6 not run)",
        f"  trades : {baseline['trades']}  vs  461  "
        f"(filter removed {461 - baseline['trades']} trades = "
        f"{(461 - baseline['trades']) / 461:.1%} of baseline)",
        "",
        f"  OVERALL: {'PASS -- Filter B improves or maintains quality' if all_gates else 'FAIL -- one or more gates failed'}",
    ]

    rpt = OUT_DIR / "filter_b_master_report.txt"
...
def main() -> int:
    print("=" * 70)
    print("Phase T17 Opt-17.2 -- Regime Filter B: Volume Confirmation")
    print("=" * 70)

    symbols = pd.read_csv(IN_SYMBOLS)["symbol"].tolist()
    print(f"Universe: {len(symbols)} symbols")
    print(f"Filter:   volume > {VOL_MULT}x prior-{VOL_MA_N}-bar MA")
    print()

    # ── STABILITY ZONE ───────────────────────────────────────────────────────
    print("[1] Stability zone (N=15/20/25, volume filter ON) ...")
    stab_rows = []
    for n in STABILITY_N:
        df_n = run_universe(entry_n=n, apply_vol=True, symbols=symbols)
        if df_n.empty:
    ...
    stab_df.to_csv(OUT_DIR / "filter_b_stability_zone.csv", index=False)

    # ── CANONICAL N=20 -- LOAD TRADES ────────────────────────────────────────
    print(f"\n[2] Canonical N={CANONICAL_N} with volume filter ...")
    trades = run_universe(entry_n=CANONICAL_N, apply_vol=True, symbols=symbols)
    if trades.empty:
        print("  ERROR: No trades generated. Check OHLCV data.")
        return 1

    trades.to_csv(OUT_DIR / "filter_b_all_trades_N20.csv", index=False)
    ...
    mc.to_csv(OUT_DIR / "filter_b_t4_montecarlo.csv", index=False)
    cost.to_csv(OUT_DIR / "filter_b_t4_cost_stress.csv", index=False)
    splits.to_csv(OUT_DIR / "filter_b_t4_period_splits.csv", index=False)
    rem_assets.to_csv(OUT_DIR / "filter_b_t4_remove_best_assets.csv", index=False)
    rem_months.to_csv(OUT_DIR / "filter_b_t4_remove_best_months.csv", index=False)
    ...
    if not acc.empty:
        ...
        eq_df.to_csv(OUT_DIR / f"filter_b_t6_equity_max{PORT_MAX_OPEN}.csv", index=False)
    ...
    print()
    print("=" * 70)
    print(f"Filter B complete -> {OUT_DIR}")
    print("=" * 70)
    return 0
