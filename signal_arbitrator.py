...
Coordinates all four T9B engines before any position entry.
...
Rules enforced:
  ...
  Rule 5 — momentum_short_conflict: spot LONG blocked by Momentum SHORT basket
"""
...
PRIORITY: dict[str, list[str]] = {
    ...
    "BEAR": ["rsi_mr",   "momentum", "donchian",   "consecdown"],
}
...
class SignalArbitrator:
    ...
    def _load_states(self) -> dict:
        ...
        for eng, path in _STATE_PATHS.items():
            ...
            try:
                ...
                by_dir: dict[str, str] = {}
                heat = 0.0
                # Spot engines (LONG only, stop-sized risk)
    ...
    def check_signal(
        ...
        # Rule 5 — Momentum short blocks spot LONG (evaluated before generic check
        #           so we can emit the specific 'momentum_short_conflict' reason)
        if direction == "LONG" and self.system != "momentum":
            mom_by_dir = self._eng.get("momentum", {}).get("by_dir", {})
            if mom_by_dir.get(norm) == "SHORT":
                return self._reject(symbol, direction, "momentum_short_conflict")
