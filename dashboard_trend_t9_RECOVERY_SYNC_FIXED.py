...
with tab7:
    ...
    for name, path in [
        ...
        rows.append({
            ...
            "path": str(path),
            "exists": path.exists(),
            "size_kb": float(round(path.stat().st_size / 1024, 2)) if path.exists() and path.is_file() else 0.0,
        })

    monitor_df = pd.DataFrame(rows)
    if "size_kb" in monitor_df.columns:
        monitor_df["size_kb"] = pd.to_numeric(monitor_df["size_kb"], errors="coerce").fillna(0.0)
    if "exists" in monitor_df.columns:
        monitor_df["exists"] = monitor_df["exists"].astype(bool)
