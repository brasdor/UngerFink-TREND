...
"""
T9C V3 — Streamlit Dashboard with Candlestick Position Charts
============================================================

Adds over V2:
- candlestick chart for each open position using Plotly
- exact entry marker on the entry candle/time
- current stop / trailing stop horizontal line
- initial stop horizontal line
- live price horizontal line
- marker/status for Chandelier trailing activation
- optional historical activation marker if T9A later logs:
    chandelier_activation_time
    chandelier_activation_price

Run:
    streamlit run dashboard_trend_t9c_V3.py

Dependencies:
    pip install streamlit pandas numpy ccxt plotly
"""
...
def profit_factor(series: pd.Series) -> float:
    ...
    return losses and gains / losses
...
def nearest_candle_time(df: pd.DataFrame, target_time):
    if df.empty or target_time is None or pd.isna(target_time):
        return None
    target = pd.to_datetime(target_time, utc=True, errors="coerce")
    if pd.isna(target):
        return None
    diffs = (df["time"] - target).abs()
    idx = diffs.idxmin()
    return df.loc[idx, "time"]


def build_candlestick_figure(symbol: str, row: pd.Series, live_price):
    if go is None:
        return None
    ...
    entry_time = pd.to_datetime(entry_time_raw, utc=True, errors="coerce")
    entry_marker_time = nearest_candle_time(df, entry_time)

    chandelier_active = str(row.get("chandelier_active", "False")).lower() in ["true", "1", "yes"]
    activation_time = None
    activation_price = None

    # Future-compatible: if T9A later stores these columns, dashboard will show exact activation point.
    if "chandelier_activation_time" in row.index:
        activation_time = pd.to_datetime(row.get("chandelier_activation_time"), utc=True, errors="coerce")
    if "chandelier_activation_price" in row.index:
        try:
            activation_price = float(row.get("chandelier_activation_price"))
        except Exception:
            activation_price = None

    if activation_time is not None and not pd.isna(activation_time):
        activation_marker_time = nearest_candle_time(df, activation_time)
    else:
        activation_marker_time = None

    if activation_price is None or pd.isna(activation_price):
        activation_price = current_stop if chandelier_active else None
    ...
    # Entry marker.
    if entry_marker_time is not None and np.isfinite(entry_price):
        ...
        fig.add_trace(
            go.Scatter(
                ...
                y=[entry_price],
                mode="markers+text",
                marker=dict(size=13, symbol=marker_symbol),
                text=["ENTRY"],
    ...
    # Current stop / trailing stop line.
    if np.isfinite(current_stop):
        fig.add_hline(
            y=current_stop,
            line_dash="dash",
            annotation_text="Current / trailing stop",
    ...
    if live_price is not None and np.isfinite(live_price):
        fig.add_hline(
            y=float(live_price),
            line_dash="dashdot",
            annotation_text="Live price",
            annotation_position="top right",
        )

    # Chandelier activation marker.
    if chandelier_active:
        if activation_marker_time is not None and activation_price is not None and np.isfinite(activation_price):
            fig.add_trace(
                go.Scatter(
                    x=[activation_marker_time],
                    y=[activation_price],
                    mode="markers+text",
                    marker=dict(size=12, symbol="star"),
                    text=["TRAIL ON"],
                    textposition="top center",
                    name="Trailing activated",
                )
            )
        else:
            # We know it is active, but T9A does not yet store the exact activation candle.
            fig.add_annotation(
                x=df["time"].iloc[-1],
                y=current_stop if np.isfinite(current_stop) else df["close"].iloc[-1],
                text="TRAILING ACTIVE<br>(exact activation time not logged yet)",
                showarrow=True,
                arrowhead=2,
            )

    title = f"{symbol} {side} — 6H candles"
...
st.set_page_config(
    page_title="Trend T9C V3 Dashboard",
    layout="wide",
)

st.title("📈 Trend Following T9C V3 — Candlestick Paper Dashboard")
st.caption("Candlestick · entry marker · current/trailing stop · live price · closed-trade equity only")
...
# ============================================================
# OPEN POSITIONS WITH CANDLESTICKS
# ============================================================

st.subheader("📌 Open Positions — Candles, Entry & Trailing Stop")

if go is None:
    st.error("Plotly non è installato. Installa con: pip install plotly")
...
if open_pos.empty:
    ...
    st.markdown("### 🕯️ Position Candlestick Charts")
    ...
    for _, row in enriched.iterrows():
        ...
        title = f"{symbol} — {side} — live {fmt_num(live, 8)}"
        with st.expander(title, expanded=True):
            ...
            with c5:
                st.metric("Trailing", "ACTIVE" if chandelier_active else "Not active")

            fig = build_candlestick_figure(symbol, row, live)
            if fig is None:
                st.warning("No candlestick chart available for this position.")
            else:
                st.plotly_chart(fig, use_container_width=True)

            if chandelier_active and "chandelier_activation_time" not in enriched.columns:
                st.info(
                    "Il trailing è attivo nello stato attuale, ma T9A non salva ancora "
                    "la candela esatta di attivazione. Per avere il punto storico preciso "
                    "serve aggiornare T9A aggiungendo chandelier_activation_time e "
                    "chandelier_activation_price."
                )
...
st.caption("T9C V3 legge i CSV/JSON locali prodotti da T9A e usa Binance solo per prezzo live/grafici. Non invia ordini.")
