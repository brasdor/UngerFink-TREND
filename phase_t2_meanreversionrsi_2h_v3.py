...
# -*- coding: utf-8 -*-
"""
Phase T2 -- MeanReversionRSI 2H v3 (os=20 fix)
...
OUT_DIR = ROOT / "data" / "research_meanreversionrsi_t2_2h_v3"
...
def main() -> None:
    ...
    p("=" * 65)
    p("  Phase T2 -- MeanReversionRSI 2H v3 (oversold fix)")
    p(f"  Timeframe : {TIMEFRAME}")
    p(f"  Config    : {config_str}")
    p(f"  Fix       : os=20 + time_exit raised 10 -> 15 (WR was 70.4%, above MR ceiling)")
    ...
    with open(OUT_DIR / "phase_t2_scorecard.txt", "w", encoding="utf-8") as f:
        f.write("=" * 65 + "\n")
        f.write("Phase T2 -- MeanReversionRSI 2H v3 (oversold fix)\n")
        f.write(f"Timeframe : {TIMEFRAME}\n")
        f.write(f"Config    : {config_str}\n")
        f.write(f"Fix       : os=20 + time_exit raised 10 -> 15 (WR was 70.4%, above MR ceiling)\n")
