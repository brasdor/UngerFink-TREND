...
"""
PHASE T9A V2 — BINANCE PAPER SIM ENGINE
====================================

Purpose
-------
Paper-live engine V2 for the Trend Following System.
...
- logs exact Chandelier activation time/price
...
def load_positions(state: dict) -> Dict[str, Position]:
    ...
    for p in state.get("open_positions", []):
        # Backward compatibility with T9A V1 state files.
...
def update_position_with_closed_bar(pos: Position, df: pd.DataFrame) -> Tuple[Optional[dict], Position]:
    ...
    if pos.side == "LONG":
        ...
        if pos.max_favorable_r >= CH_ACTIVATE_R and np.isfinite(atr):
            was_active = pos.chandelier_active
...
def try_open_position(signal: dict, state: dict, positions: Dict[str, Position]) -> Tuple[Optional[Position], Optional[dict]]:
    ...
    pos = Position(
        ...
        chandelier_activation_time=None,
        chandelier_activation_price=None,
...
def main() -> int:
    print("=" * 80)
    print("PHASE T9A V2 — BINANCE PAPER SIM ENGINE")
    print("=" * 80)
